...
import {
  ...
  GripVertical, Zap, Flame, Activity, ChevronDown
} from 'lucide-react';
...
const CustomWorkoutBuilder: React.FC<CustomWorkoutBuilderProps> = ({
  onBack, onSave, isDarkMode, themeColor
}) => {
      ...
      {(['info', 'exercises', 'review'] as const).map((s, i) => {
        ...
        return (
          <React.Fragment key={s}>
            ...
            {i < 2 && <div className={`h-px w-8 transition-all ${done ? accentBg.replace('bg-', 'bg-') : 'bg-zinc-800'}`}
              style={{ backgroundColor: done ? accentHex : '#27272a' }} />}
          </React.Fragment>
        );
      })}
    </div>
              ...
              {showSuggestions === ex.id && ex.name.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-zinc-800 rounded-xl border border-zinc-700 shadow-2xl z-20 max-h-36 overflow-y-auto">
                  {EXERCISE_SUGGESTIONS.filter(s =>
                    s.toLowerCase().includes(ex.name.toLowerCase()) && s !== ex.name
                  ).slice(0, 5).map(s => (
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex flex-col">
      ...
      <div className="px-5 pt-14 pb-4 flex items-center gap-3 border-b border-zinc-800/60 shrink-0">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-black">Crea Scheda</h1>
      ...
      <div className="px-5 py-5 border-t border-zinc-800/50 shrink-0 bg-[#080808]">
        <div className="flex gap-3">
          ...
          <button
            ...
            {saved ? (
              <><Check size={16} strokeWidth={3} /> Salvata!</>
            ) : step === 'review' ? (
              <><Check size={16} strokeWidth={3} /> Salva Scheda</>
            ) : (
